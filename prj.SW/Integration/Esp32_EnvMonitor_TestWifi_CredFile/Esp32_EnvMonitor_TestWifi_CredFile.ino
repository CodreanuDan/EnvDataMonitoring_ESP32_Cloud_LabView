...
float           SmokeCurve[3] ={2.3,0.53,-0.25};    
...
void LimitAlertCheck(SensorData* data, VolumeSettings* settings)
{
  ...
  else
  {
    ...
    Wifi_SendDataToGoogleSpreadsheets(data);
...
/****************************  Wifi_LoadNetworks ***************************
  Input:   None
  Output:  None
  Remarks: This function loads the file that contains wifi credentials from SPIFFS
************************************************************************************/ 
void Wifi_LoadNetworks() 
{
    ...
    if (!file) 
    {
        Serial.println("[SPIFFS] Failed to open WiFi config file");
        return;
    }
    ...
    Serial.println("[SPIFFS] Networks loaded:");
    for (auto &network : wifiNetworks) 
    {
        Serial.println("  - SSID: " + network.first);
    }
}
...
bool Wifi_SetupNetworkConnection() 
{
    ...
    /* Find the strongest saved network */ 
    for (int i = 0; i < n; ++i) 
    {
        String currentSSID = WiFi.SSID(i);
        int signalStrength = WiFi.RSSI(i);
        Serial.printf("  [%d] %s (RSSI: %d)\n", i + 1, currentSSID.c_str(), signalStrength);

        if (wifiNetworks.count(currentSSID) > 0 && signalStrength > strongestSignal) 
        {
            strongestSignal = signalStrength;
            bestSSID = currentSSID;
            bestPassword = wifiNetworks[currentSSID];
        }
    }

    /* Try connecting to the best network found */ 
    if (!bestSSID.isEmpty()) 
    {
    ...
    {
        Serial.println("[WiFi] No saved networks found among scanned results.");
    }
